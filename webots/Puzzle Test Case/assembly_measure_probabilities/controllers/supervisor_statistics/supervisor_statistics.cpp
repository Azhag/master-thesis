...
// #define MEASPIECEARENA2
// #define MEASPIECEARENA3
// #define MEASROBOTARENA2
#define MEASROBOTARENA3
	...
	// The arena radius
	#define ARENA_RADIUS 2*0.95
	// To end the experiments quicker
	#define ONEREACTION
#endif
#ifdef MEASPIECEARENA3
	// The number of pieces that can be assembled maximum
	#define NB_ASSEMBLIES 1
	// The number of robots
	#define NB_ROBOTS 1
	...
	#define NB_PIECES 1
	// The arena radius
	#define ARENA_RADIUS 3.0*0.95
	// To end the experiments quicker
	#define ONEREACTION
#endif
#ifdef MEASROBOTARENA2
	// The number of pieces that can be assembled maximum
	#define NB_ASSEMBLIES 1
	// The number of robots
	#define NB_ROBOTS 2
	...
	#define NB_PIECES 0
	// The arena radius
	#define ARENA_RADIUS 2.0*0.95
	// To end the experiments quicker
	#define ONEREACTION
#endif
#ifdef MEASROBOTARENA3
	// The number of pieces that can be assembled maximum
	#define NB_ASSEMBLIES 1
	// The number of robots
	#define NB_ROBOTS 15
...
void parseMsgFinalAssembly(char* recMessage, float* time_encountering, int* is_assembly) {
	...
	*time_encountering = atof(parsedMsg);
	...
	// Go for the last number.
	while(i< 15 && parsedMsg != NULL) {
		*is_assembly = atoi(parsedMsg); // If there is a 1 in the last position, we just constructed a new puzzle
		parsedMsg = strtok(NULL, " ");
		i++;
	}
}
...
void run_supervisor() {
	...
	*/
	if (receiver_get_queue_length(receiver) > 0) {
		...
		#ifdef ONEREACTION
			// When we measure the piece encounter probability or the robot encounter probability, 
			// the first message received is already enough, we can stop there.
			new_assembly_finished = 1.0;
		#endif
