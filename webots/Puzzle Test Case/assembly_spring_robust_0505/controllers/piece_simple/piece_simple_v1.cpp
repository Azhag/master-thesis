...
// piece states
enum { FREE, CLAIMED, ROTATING, CARRIED, PIECE_5, PIECE_6, PIECE_7, PIECE_8 };
int state = FREE;
...
static int run(int ms)
{
   ...
   //  }
     ...
     if (receiver_get_queue_length(commReceiver)) {
     	...
     	const void *buffer = receiver_get_data(commReceiver);
	  	...
	  	if (recMessage[0] == 'r'){ // add condition(s)
   ...
   case CARRIED:
   	...
   	if (receiver_get_queue_length(commReceiver)) {
        const void *buffer = receiver_get_data(commReceiver);
   	  	...
   	  	if (recMessage[0] == 'r' && recMessage[2] == '3'){ // add condition(s)
   ...
   case PIECE_6:  // 6th type of piece: 3 and 4
   	...
   	if (receiver_get_queue_length(commReceiver)) {
           const void *buffer = receiver_get_data(commReceiver);
   			...
   			emitter_send_packet(commEmitter, "Lock6", strlen("Lock6") + 1);
   		}
   		receiver_next_packet(commReceiver);
   	  }
   ...
   case PIECE_7:  // 7th type of piece: 1, 2, 3, 4
   	  ...
   	  if (receiver_get_queue_length(commReceiver)) {
			...
			emitter_send_packet(commEmitter, "Lock7", strlen("Lock7") + 1);
			}
			receiver_next_packet(commReceiver);
   	  }
	...
	if (receiver_get_queue_length(commReceiver)) {
		...
		if (recMessage[0] == 'r' && recMessage[2] == '7' &&
		  ...
		  // robot does not yet think its piece type is state 8
			emitter_send_packet(commEmitter, "Lock8", strlen("Lock8") + 1);
		}
		receiver_next_packet(commReceiver);
   	}
