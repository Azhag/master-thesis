...
static int run(int ms)
{
	...
	char lockMessage[20];  // message from locked piece
  ...
  // waitCtr needed
		...
		while (receiver_get_queue_length(commReceiver) > 0) {
			...
			if (recMessage[0] == 'p' && recMessage[1] == pieceType && recMessage[3] == pieceNum){
    ...
    if (receiver_get_queue_length(commReceiver) == 0) {
      ...
      if (waitCtr > WAIT_TIME) {
		...
		while (receiver_get_queue_length(commReceiver) > 0) {
			...
			// MORE CONDITIONS?
			if (recMessage[0] == 'r' && recMessage[1] == otherRobotID){  // it's a robot (not a piece)
    ...
    if (receiver_get_queue_length(commReceiver) == 0) {
      ...
      if (waitCtr > WAIT_TIME) {
		...
		while (receiver_get_queue_length(commReceiver) > 0) {
			...
			speed[0] = 0;
			speed[1] = 0;
			if (robot_num == robot2 && recMessage[0] == 'r' && recMessage[1] == otherRobotID
          && char2state(recMessage[2])==WAIT) {
        waitCtr = 0;
        abortCtr = 0;
				state = ALIGN_WITH_ROBOT;  // robot 1 has finished aligning
			}
			if (robot_num == robot1 && recMessage[0] == 'r' && recMessage[1] == otherRobotID
          && char2state(recMessage[2])==APPROACH_ROBOT) {
        waitCtr = 0;
        abortCtr = 0;
	...
	case APPROACH_ROBOT:
		...
		while (receiver_get_queue_length(commReceiver) > 0) {
			...
			if (recMessage[0] == 'L' && recMessage[5] == robot_num) {
					// the piece that it's carrying
				speed[0] = 0;
				speed[1] = 0;
