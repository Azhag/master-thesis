...
int piece_type_nb_connectors[] = {1, 3, 2, 1};
...
static int run(int ms)
{
	...
	if (nb_moves <= 0) {
		...
		dx = 0.01*((rand() / (float)RAND_MAX)-0.5);
		dy = 0.01*((rand() / (float)RAND_MAX) -0.5);

		custom_robot_set_rel_force_and_torque(dx, 0.0, dy, 0.0, 0.0, 0.0);
