...
class WorldHandler():
	...
	def handleRobot(self, robot):
		...
		robotType = robot.getElementsByTagName("Type")[0].lastChild.data;
		...
		# Place the robots
		self.builder.constructRobots(robotType, numberRobots, controller, initialPosRand, x, y);
	##
	#########################################################
	...
	def handlePiece(self, piece, nbBlocks):
		...
		coulombFriction = piece.getElementsByTagName("CoulombFriction")[0].lastChild.data;
		...
		# Place the pieces
		self.builder.constructPieces(nbBlocks, pieceType, nbPieces, self.pieceNumStart, controller, coulombFriction, initialPosRand, x, y);
		
		self.pieceNumStart += nbPieces;
	##
	#########################################################
